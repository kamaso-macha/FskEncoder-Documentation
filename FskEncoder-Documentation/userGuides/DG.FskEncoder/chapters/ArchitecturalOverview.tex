...
\begin{figure}[h!]
	...
	\includegraphics[width=\linewidth, keepaspectratio]{../_images/ArchitectureOverview.png}
...
\begin{minipage}{0.45\textwidth}
	...
	\\
	The top-one, with the caption \textit{Region}\, is the extension GUI of the input reader extension while the lower one (with the field \textit{File Name}\,) is from the target system extension.
	\\
...
\begin{minipage}{0.45\textwidth}
	...
	\adjustimage{width=\textwidth, valign=t}{../_images/DG.Mainwindow.png}
...
Figure \fullref{fig:architecture-overview} depicts the global structure of the application together with the extensions:

\begin{itemize}

	\item \textbf{View:} This is the main window with all GUI elements exept that of the GUI extensions.

	\item \textbf{Control:} This is the control for the main window and deals with every thing exept the extension GUIs.

	\item \textbf{Workflow engine:} Encapsulates the \textit{business logic}\, of the FskEncoder application and the extensions.

	\item \textbf{Model:} The data model of the application. It holds some run-time data and the properties of the two configuration files \textit{FskEncoder.properties}\, and \textit{Plugin.properties}\,.
	...
	\\
	The example in figure \ref{fig:main-window} on top of this page shows the GUI of the Ihx8InputReaderExtension which can handle more than one memory section in a single file.
...
A deep dive into how to build those extensions is discussed in the section \fullref{extension-instantiation}.
\\
...
This package provides the logic needed for extending the application. A deeper discussion can be found in section \fullref{api}.
\\
The content of this package is packed in a seperate file \textit{FskEncoderExtension.jar} \, during build process and \must be used as library in the FskEncoder-Extensions part.
...
Please refer to appendix \fullref{ProtocolImplementation} for more details.


%*******************************************************************************

\subsubsection{extension.execution}

\public

Contains classes and interfaces needed for background task execution of target system extension protocols.
